...
// </editor-fold>
        ...
        for (Village v : radarVillages) {
            try {
                ...
                for (UnitHolder u : DataHolder.getSingleton().getUnits()) {
                    ...
                    Color co = Color.decode(GlobalOptions.getProperty(u.getName() + ".color"));
                    c.setDrawColor(co);
                    c.setDrawAlpha(0.8f);
                    c.renderForm(g2d);
        ...
        if (!mouseDown && mouseVillage != null && GlobalOptions.getProperties().getBoolean("show.mouseover.info")) {
        ...
        if (!MapPanel.getSingleton().isMouseDown()
                && GlobalOptions.getProperties().getBoolean("show.map.popup")
